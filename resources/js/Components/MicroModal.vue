...
// 顧客検索
// DBにアクセスし取得するまでに時間がかかるからasync-await(非同期関数)を使用して取得後に表示させる
const search = ref('')
const customers = reactive({})

const isShow = ref(false)
const toggleStatus = () => { isShow.value = !isShow.value} //モーダル表示

const searchCustomers = async () => { //async
  try {
  await axios.get(`/api/searchCustomers/?search=${search.value}`) //await:16行目から20行目までの処理が終わったら次に進む
  .then( res => {
    console.log(res.data)
    customers.value = res.data //検索するをクリックして情報の取得が出来たらcustomersに値が代入される
  })
  toggleStatus()
  } catch(e){
    console.log(e)
...
<template>
    ...
    <button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="mt-4 flex mx-auto text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg">検索する</button>
</template>